...
│   │  - 最適化: Adam（lr=1e-4）                                   │ │
│   │  - 画像サイズ: 384×512（H×W）にリサイズ ※正方形ではない    │ │
│   │  - バッチサイズ: 8（GPUメモリ依存）                          │ │
│   └──────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
...
### 実マンガからの自己拡張学習（Self-Augmentation from Real Manga）

少数の実マンガアノテーションデータを活用して、Domain Gapを埋めるデータ拡張手法のアイデア。

#### 背景と着想

現状の課題:
- 合成データ（生成AI背景 + 生成AI吹き出し）で学習
- 実マンガとの見た目の違い（Domain Gap）が汎化性能のボトルネック

解決アイデア:
- 少数でも実マンガのアノテーションがあれば、それを「分解・再合成」してデータ拡張できる

#### 処理パイプライン

```
【入力】実マンガ画像 + 吹き出しマスク（アノテーション済み）
         例: 200ページ分
...
┌─────────────────────────────────────────────────────────────┐
│  1. 背景抽出（Inpainting）                                   │
│     - 吹き出し領域をマスクとしてinpainting                  │
│     - 吹き出しが除去された「実マンガ背景」を取得            │
│     - ツール: OpenCV inpaint, LaMa, Stable Diffusion        │
│                                                              │
│  2. 吹き出し抽出                                             │
│     - マスク領域から吹き出しを切り出し                      │
│     - 透過PNG化                                              │
...
┌─────────────────────────────────────────────────────────────┐
│  3. 自己拡張データセット生成                                 │
│     - 実マンガ背景 × 実マンガ吹き出し を再合成              │
│     - 配置・サイズは統計ベースでランダム化                  │
│     - 元の組み合わせとは異なるペアを生成                    │
│                                                              │
│  例: 200ページ → 200背景 × 1000+吹き出し                    │
│      → 数千〜数万の新しい学習データ                         │
└─────────────────────────────────────────────────────────────┘
```
...
| 比較項目 | 従来（生成AI合成） | 提案（実マンガ自己拡張） |
|----------|-------------------|-------------------------|
| 背景の質 | 生成AIの癖がある | 実マンガそのもの |
| 吹き出しの質 | 形状が単純化 | 実際の多様な形状 |
| Domain Gap | 大きい | 小さい |
| 必要データ | なし | 少数のアノテーション |
...
def extract_background(image, mask):
    """吹き出しをinpaintingで除去して背景を得る"""
    # OpenCV inpaint（シンプル版）
    background = cv2.inpaint(image, mask, 3, cv2.INPAINT_TELEA)
    return background

# 2. 吹き出し切り出し
def extract_balloons(image, mask):
    """マスクから個別の吹き出しを切り出し"""
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    balloons = []
    for cnt in contours:
        x, y, w, h = cv2.boundingRect(cnt)
        balloon_rgba = create_transparent_balloon(image, mask, x, y, w, h)
        balloons.append(balloon_rgba)
    return balloons

# 3. 再合成（既存のcreate_syn_balloon_dataset.pyを流用）
# --balloon-dir に抽出した実吹き出しフォルダを指定
# --back-dir に抽出した実背景フォルダを指定
```
...
Phase 2: パイプライン自動化
  - inpaint_real_manga.py 作成
  - extract_real_balloons.py 拡張
...
1. **Inpainting品質**: 吹き出し除去後の背景が不自然になる可能性
   - 対策: 高品質なinpaintingモデル（LaMa等）の使用

2. **吹き出し境界**: 切り出し時に背景が混入する可能性
   - 対策: マスクを数ピクセル収縮してから切り出し

3. **ライセンス**: Manga109等のデータ利用規約に注意

#### 関連ファイル（既存）

- `extract_real_balloons.py`: Manga109から吹き出し抽出（拡張可能）
- `inpaint_openai.py`: OpenAI APIでのinpainting（参考）
- `create_synreal_dataset.py`: 合成+実データ統合（参考）
